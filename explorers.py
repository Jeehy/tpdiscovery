...
class ResearchExplorers:
    ...
    def run_path_omics_driven(self, threshold=6.0, disease="liver cancer"):
        ...
        # 3. OpenTargets 批量快速扫描 (检查新颖性)
        print(f"   -> 正在 OpenTargets 数据库中核查 {len(gene_list)} 个基因的已知程度...")
        ot_res = self.opentargets.run({"topic": disease, "genes": gene_list})
        ot_scores = {item['symbol']: item['score'] for item in ot_res.get('results', [])}
        ...
        print(f"   -> 剔除 KG 和 OpenTargets 已知靶点后，Path A 保留 {len(results)} 个新颖候选")
        return results
    ...
    def run_validation_deep_diven(self, target_list: list, disease="liver cancer"):
        ...
        ot_map = {item['symbol']: item for item in ot_res.get('results', [])}

        results = {}
        for gene in target_list:
            ot_info = ot_map.get(gene, {"score": 0.0})
